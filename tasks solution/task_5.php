...
class task
{
    ...
    public function login($username , $pass , $tabe_name)
    {
        ...
        if($this->is_email($username,"gmail.com") || $this->is_email($username , "yahoo.com"))
        {
            ...
            {
                $select = $this->con->query("SELECT  password FROM ".$this->table_name." WHERE username ='" . $email . "' AND password ='" . $this->pass . "'");
            ...
            {
                $select = $this->con->query('SELECT  password FROM '.$this->table_name.' WHERE username ="' . $email . '"');
                if ($select->num_rows == 1)
                {
        ...
        else if($this->is_phone_number($username))
        {
            ...
            {
                $select = $this->con->query('SELECT  password FROM '.$this->table_name.' WHERE username ="' . $username . '"  AND password ="'. $this->pass .'"');
            ...
            {
                $select = $this->con->query('SELECT  password FROM '.$this->table_name.' WHERE username ="' . $username . '"');
